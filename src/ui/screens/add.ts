...
export async function Add(root: HTMLElement) {
    ...
    const render = () => {
      ...
      <section class="space-y-4">
        ...
        <a href="#meditation" class="block p-4 rounded-xl bg-ink-700 border border-butter-300/20 hover:bg-ink-900 transition-all group">
            <div class="flex items-center gap-3">
                <div class="text-2xl">🪷</div>
                <div class="flex-1">
                    <div class="font-medium">Meditation Timer</div>
                    <div class="text-xs opacity-70">Start a timed meditation session</div>
                </div>
                <div class="text-butter-300 opacity-0 group-hover:opacity-100 transition-opacity">Start →</div>
            </div>
        </a>
